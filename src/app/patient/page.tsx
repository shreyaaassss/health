  ...
  return (
    <div className="px-4 pt-6 pb-4" style={{ background: 'var(--page)', minHeight: '100%' }}>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <p style={{ fontSize: 12, fontWeight: 600, color: '#8A93A3' }}>{greeting}</p>
          <p style={{ fontSize: 20, fontWeight: 800, color: '#12151C', marginTop: 2 }}>Hello, {firstName}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            <svg viewBox="0 0 24 24" width={18} height={18} fill="none" stroke="#4B5265" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
      ...
      <div
        className="mb-4 p-5"
        style={{ background: '#FFFFFF', borderRadius: 26 }}
      >
        <div className="flex items-center gap-5">
          {/* Ring SVG */}
          <div className="relative flex-shrink-0" style={{ width: 88, height: 88 }}>
            <svg width="88" height="88" viewBox="0 0 88 88" style={{ transform: 'rotate(-90deg)' }}>
              <circle cx="44" cy="44" r="38" fill="none" stroke="#EEF1F6" strokeWidth="9"/>
              <circle cx="44" cy="44" r="38" fill="none" stroke="#2F6BFF" strokeWidth="9" strokeLinecap="round"
                strokeDasharray={circumference}
                strokeDashoffset={dashOffset}/>
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span style={{ fontSize: 18, fontWeight: 800, color: '#12151C' }}>{pct}%</span>
            </div>
          </div>
          <div className="flex-1 min-w-0">
            <p style={{ fontSize: 10, fontWeight: 700, color: '#8A93A3', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Health Wallet</p>
            <p style={{ fontSize: 16, fontWeight: 800, color: '#12151C', marginTop: 4, lineHeight: 1.3 }}>Your records, your control</p>
            <p style={{ fontSize: 12, color: '#8A93A3', marginTop: 4 }}>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <Link
          ...
          style={{ background: '#FFFFFF', borderRadius: 18, padding: '14px 16px', display: 'block' }}
        >
          <div
          ...
          <p style={{ fontSize: 24, fontWeight: 800, color: '#12151C' }}>{recordCount}</p>
          <p style={{ fontSize: 12, color: '#8A93A3', marginTop: 2 }}>Medical Record{recordCount !== 1 ? 's' : ''}</p>
        </Link>
        <Link
          href="/patient/access"
          className="active:scale-[0.98] transition-transform"
        ...
        >
          ...
          <div className="flex items-center gap-1.5">
            <p style={{ fontSize: 24, fontWeight: 800, color: '#12151C' }}>{activeGrants}</p>
            {activeGrants > 0 && (
              <span className="w-2 h-2 rounded-full animate-pulse" style={{ background: '#1FAA6D', marginTop: 2 }} />
            )}
          </div>
          <p style={{ fontSize: 12, color: '#8A93A3', marginTop: 2 }}>Active Share{activeGrants !== 1 ? 's' : ''}</p>
        </Link>
      </div>
      ...
      {firstGrant && (
        <div className="mb-4">
          <p style={{ fontSize: 12, fontWeight: 700, color: '#8A93A3', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Active Access</p>
          <div
            style={{ background: '#FFFFFF', borderRadius: 18, border: '1.5px solid #EAF1FF', padding: '14px 16px' }}
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="truncate" style={{ fontSize: 14, fontWeight: 700, color: '#12151C' }}>{firstGrant.provider.name}</p>
                <p className="truncate" style={{ fontSize: 12, color: '#8A93A3' }}>{firstGrant.provider.organization}</p>
              </div>
              {/* LIVE badge */}
              <span
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
              {firstGrant.records.slice(0, 3).map((r) => {
                ...
                return (
                  <span
                    key={r.id}
                    style={{ background: '#F3F8FF', color: '#1D4FE0', fontSize: 11, fontWeight: 600, borderRadius: 10, padding: '3px 8px' }}
                  >
                    {r.title}
                  </span>
                );
              ...
              {firstGrant.records.length > 3 && (
                <span style={{ background: '#F3F8FF', color: '#8A93A3', fontSize: 11, fontWeight: 600, borderRadius: 10, padding: '3px 8px' }}>
            ...
            <div className="flex items-center justify-between">
              <p style={{ fontSize: 11, color: '#8A93A3' }}>
                {(() => {
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between mb-3">
          <p style={{ fontSize: 14, fontWeight: 700, color: '#12151C' }}>My health records</p>
          <Link href="/patient/records" style={{ fontSize: 13, fontWeight: 600, color: '#2F6BFF' }}>See all</Link>
        </div>
        {recentRecords.length === 0 ? (
          <p style={{ fontSize: 13, color: '#8A93A3', textAlign: 'center', padding: '24px 0' }}>No records yet</p>
        ) : (
